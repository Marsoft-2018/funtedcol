<?php
    ...
    public function listar(){
        $this->sql = "SELECT * FROM categorias_programas ORDER BY id ASC";
        try {
    ...
    public function cargar(){
        $this->sql = "SELECT * FROM categorias_programas WHERE id = ?";
        try {
            $stm = $this->conexion->prepare($this->sql);
    ...
    public function agregar(){
        $this->sql = "INSERT INTO categorias_programas(nombre,descripcion) VALUES(?,?)";
          try {
            $stm = $this->conexion->prepare($this->sql);
    ...
    public function modificar(){
        $this->sql = "UPDATE categorias_programas SET nombre =?, descripcion =? WHERE id = ?";
          try {
            $stm = $this->conexion->prepare($this->sql);
            $stm->bindparam(1,$this->nombre);
            $stm->bindparam(2,$this->descripcion);
    ...
    public function eliminar(){
        $this->sql = "DELETE FROM categorias_programas WHERE id = ?";
          try {
            $stm = $this->conexion->prepare($this->sql);
            $stm->bindparam(1,$this->id);         
            if($stm->execute()){
